...
@implementation Anunciantes
@synthesize scrollView;
@synthesize pageControl;
@synthesize widhtall,heihtall,anunciosarray,detailViewController,queue;
    ...
    if(sqlite3_open([appDelegate.databasePath UTF8String], &database) == SQLITE_OK) {
        ...
        if(sqlite3_prepare_v2(database, [sqlStatement UTF8String], -1, &compiledStatement, NULL) == SQLITE_OK) {
            while(sqlite3_step(compiledStatement) == SQLITE_ROW) {
                ...
                NSString *turstico = [NSString stringWithUTF8String:(char *)sqlite3_column_text(compiledStatement, 3)];
                ...
                auxanuncios.turistico = [turstico intValue];
...
-(void)configuraciones{
	...
	self.scrollView.contentSize = CGSizeMake(ancho * (anunciosarray.count/2), alto);
	self.scrollView.delegate = self;
	self.pageControl.currentPage = 0;
	self.pageControl.numberOfPages = (anunciosarray.count/2);
